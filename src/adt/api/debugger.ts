...
export interface DebuggerListenOptions {
    /** Cancels the pending long-poll request without issuing another ADT action. */
    signal?: AbortSignal
    /** Overrides the historical 100-hour listener timeout. */
    timeout?: number
}
...
export async function debuggerListen(
    ...
    const response = await h.request("/sap/bc/adt/debugger/listeners", {
        method: "POST",
        timeout: options?.timeout ?? 360000000, // 100 hours
        qs,
