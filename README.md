...
Phase 2 now tracks thirteen additional source/service kinds in the same capability catalog: class, interface, include, four CDS source types, service definition, behavior definition, CDS type, CDS aspect, CDS entity buffer, and service binding. All thirteen are `AUTOMATION_VERIFIED` with controlled preview schemas and `available=true`/`writable=false`. `DDIC_STRUCTURE`, `DDIC_TYPE_GROUP`, and `DDIC_LOCK_OBJECT` are also wired into the controlled lifecycle at `CONTROLLED_IMPLEMENTED`; their creation media types are resolved from ADT discovery and remain non-writable until real DEV validation. Type Group creation is fixed to `TYPE/DG`, a maximum five-character name, and a matching `TYPE-POOL` source. Lock Object creation is structured rather than source-based, fixes `allowRFC=false` and an empty lock mode, and requires an existing primary table; it has no separate activation stage. CDS Type uses `DRTY/STY` (maximum 40-character name), CDS Aspect uses `DRAS/RAS`, and CDS Entity Buffer uses `DTEB/DF` with an existing active CDS entity reference plus controlled `layer`/`type` source clauses. DCL, metadata-extension, service-definition, behavior-definition, and entity-buffer plans bind active CDS references and revalidate them at apply time; service bindings bind an active service definition and revalidate their OData version/category configuration. Service creation is fixed to the definition variant, while behavior extensions remain excluded and a behavior definition must use the same repository name as its root CDS entity. Eclipse ADT 3.60.2 class defaults are fixed to public and final. No real SAP write has been performed for these slices.

The complete target is now measured from installed Eclipse ADT 3.60.2 ABAP New Wizards, not only explicit `creationAdapter` extensions. The local JAR audit found 142 wizard candidate types and 28 explicit adapters; the current capability catalog controls 31 candidates, leaves 111 for protocol extraction, and has 0 `REAL_DEV_VERIFIED` types. `FUGR/I` is represented as a controlled suffix-based function-group include capability, while `DTEL/DE`, `DOMA/DD`, and `MSAG/N` now have controlled shell/property/source/activation adapters. `TTYP/DA` now has a controlled structured XML lifecycle; its predefined row types and length/decimal bounds are frozen from the target `/sap/bc/adt/ddic/codecompletion?type=abapType` response, including `CURR` and `QUAN`. All remain non-writable until real DEV verification. `VIEW/DV` remains in protocol extraction. See the [ADT 3.60.2 creation wizard manifest](docs/evidence/eclipse-adt-3.60.2-creation-wizard-manifest.json); run `npm run check:repository-creation-coverage` for code/manifest consistency, or set `SAP_ADT_ECLIPSE_PLUGINS` and run `npm run audit:eclipse-adt-creation` to recheck the installed JARs.
...
`CHANGE_DOCUMENT_OBJECT` (`CHDO/CHD`) is one of 31 registered object kinds and now has a complete controlled server-driven slice at `CONTROLLED_IMPLEMENTED`, `available=true`, and `writable=false`. Public input is limited to the 15-character repository name, category, package/transport, reviewed table-or-structure entries, optional currency/quantity reference tables, logging choices, and an existing message class/three-digit number. Preview freezes the Blue v1/objectTypes.v1 contract and all active references; the public `behaviorDefinition` value is mapped internally to SAP ADT 3.60.2's published `behaviorDefiniton` spelling. Apply creates one shell, writes only the reviewed JSON, rereads working content, unlocks, activates once, and requires an active generated `FUGR/FF` for `standard` or `CLAS/OC` for `behaviorDefinition`. Because activation can generate another repository object, any activation or post-activation verification uncertainty enters no-retry `OUTCOME_UNKNOWN` and never auto-deletes. JAR behavior, target DEV discovery/schema/configuration, and 20 active examples were verified read-only; no real CHDO creation, activation, generated-object deletion, or cleanup was performed.
...
- Automated verification: 106 Jest suites and 687 tests cover the 201-call embedded ADTClient surface, high-level read tools, controlled quality and repository-creation workflows, typed class/interface/include/CDS/service-binding/DESD/NROB/RONT/NONT/CHDO ADT contracts, Windows confirmation, gate-deadlock regression, frozen package identity, function-group Include campaign validation, DDIC defaults, validation ASX, and package discovery constraints, 21 raw tools, six controlled advanced tools, safe source/debug workflows, profile/role dispatch, request/response limits, bounded state, source normalization, SM21 behavior, logging, and serialized audit writes.
