...
Phase 2 tracks thirteen additional source/service kinds in the same capability catalog. Eleven now have complete real DEV evidence: class, interface, include, CDS data definition, DCL, metadata extension, service definition, behavior definition, CDS type, CDS aspect, and service binding. Annotation definition and CDS entity buffer remain non-writable. DCL, metadata-extension, service-definition, behavior-definition, and entity-buffer plans bind active CDS references and revalidate them at apply time; service bindings bind an active service definition, activate explicitly, and verify OData version/category plus publication state.

The complete target is measured from installed Eclipse ADT 3.60.2 ABAP New Wizards. The current catalog controls 31 candidates, leaves 111 for protocol extraction, and has 28 kinds at `REAL_DEV_VERIFIED`; 3 remain non-writable. See the lifecycle evidence under `docs/evidence/` and the [ADT wizard manifest](docs/evidence/eclipse-adt-3.60.2-creation-wizard-manifest.json).
