...
固定范围：SAP DEV client 300；包 `Z001`；传输 `S4HK900009`；前缀 `ZV`。每行最多一次 apply；历史计划不得复用。产品化后续清理均使用独立原生确认。
...
| 22 | `CDS_DATA_DEFINITION` | `ZVPCDS02` | active CDS root | `REAL_DEV_VERIFIED`：APPLIED、active、同一未释放传输 neutral CTS、DELETE、absence 完整 |
| 23 | `CDS_ACCESS_CONTROL` | `ZVPDCL02` | `ZVPCDS02` active | `REAL_DEV_VERIFIED`：APPLIED、active、neutral CTS、DELETE、absence 完整 |
| 24 | `CDS_METADATA_EXTENSION` | `ZVPMDE03` | `ZVPCDS03` active | `REAL_DEV_VERIFIED`：APPLIED、active、neutral CTS、DELETE、absence 完整 |
| 25 | `SERVICE_DEFINITION` | `ZVPSRV01` | `ZVPCDS03` active | `REAL_DEV_VERIFIED`：APPLIED、active、neutral CTS、DELETE、absence 完整 |
| 26 | `BEHAVIOR_DEFINITION` | `ZVPCDS05` | 同名 root entity active | `REAL_DEV_VERIFIED`：APPLIED、active、neutral CTS、DELETE、absence 完整 |
| 27 | `CDS_ENTITY_BUFFER` | `ZVPBUF01` | `ZVPCDS05` active | 引用实体不可 buffer，已补偿；未晋级 |
| 28 | `SERVICE_BINDING` | `ZVPSVB03` | `ZVPSRV01` active | `REAL_DEV_VERIFIED`：v2 create、activation、active 配置、`SRVB/G4BA` neutral CTS 与 absence 完整 |
...
| 31 | `CHANGE_DOCUMENT_OBJECT` | `ZVPCHDO05` | active `T000`、隐藏默认 `CD/600` | `REAL_DEV_VERIFIED`：正式 `APPLIED`、active CHDO/Class、父对象单次 DELETE、级联 absence、`CHDO/CLAS` neutral CTS 完整；旧 `ZVPCHDO04` unknown 身份不复用 |
